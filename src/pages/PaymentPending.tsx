  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Show both pending_payment and payment_not_received
        setDispatchNotes(notes.filter((n: any) => n.status !== 'payment_received'));
      }
    } catch (err) {
      console.error(err);
    } finally {
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">No pending payments found.</td></tr>
              ) : (
                  ...
                  <tr key={note.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        note.status === 'payment_not_received' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'
                      }`}>
            ...
            <div className="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
              <form onSubmit={handlePayment}>
                ...
                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                  <button type="submit" className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm bg-green-600 hover:bg-green-700 focus:ring-green-500">
                    Submit
                  </button>
